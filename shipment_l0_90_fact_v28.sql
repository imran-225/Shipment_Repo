...
LEFT OUTER JOIN (SELECT `data`.pincode as src_pincode, `data`.zone as src_zone, `data`.local_territory as src_lt,`data`.city as src_city,`data`.state as src_state from bigfoot_snapshot.dart_fki_scp_ekl_geo_1_view_total) geo_src ON If(S_current.`data`.shipment_type = 'rvp',S_current.`data`.destination_address.pincode,S_current.`data`.source_address.pincode) = geo_src.src_pincode
LEFT OUTER JOIN (SELECT `data`.pincode as dest_pincode, `data`.zone as dest_zone, `data`.local_territory as dest_lt,`data`.city as dest_city,`data`.state as dest_state from bigfoot_snapshot.dart_fki_scp_ekl_geo_1_view_total) geo_dest ON If(S_current.`data`.shipment_type = 'rvp',S_current.`data`.source_address.pincode,S_current.`data`.destination_address.pincode) = geo_dest.dest_pincode
LEFT OUTER JOIN 
bigfoot_external_neo.scp_ekl__fc_profiler_volumetric_estimate_final_hive_fact Vol 
...
group by refid) b2CLogisticsRequest ON (S_current.entityid = b2CLogisticsRequest.shipment_id)
LEFT OUTER JOIN
...
LEFT OUTER JOIN 
(select entityid, `data`.shipment_type as shipment_type, lower(`data`.status) as status from bigfoot_snapshot.dart_wsr_scp_ekl_shipment_4_view where `data`.shipment_type in ('approved_rto','unapproved_rto','rvp') and lower(`data`.status) NOT IN ('reshipped', 'pickup_leg_completed')) T1 on (T1.entityid = S_current.`data`.associated_shipment_ids[0])
LEFT OUTER JOIN 
...
from bigfoot_journal.dart_fkint_scp_ekl_geo_classification_2) lzn_cons on (If(S_current.`data`.shipment_type = 'rvp',S_current.`data`.destination_address.pincode,S_current.`data`.source_address.pincode) = lzn_cons.source_pincode and If(S_current.`data`.shipment_type = 'rvp',S_current.`data`.source_address.pincode,S_current.`data`.destination_address.pincode) = lzn_cons.destination_pincode)
WHERE
