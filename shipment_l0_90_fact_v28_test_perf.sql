...
If(size(S_current.`data`.associated_shipment_ids)=1,S_current.`data`.associated_shipment_ids[0],NULL) as first_associated_shipment_id,
...
if(lower(S_current.`data`.vendor_tracking_id) like '%myn%','MYN',
if(upper(S_current.`data`.vendor_tracking_id) like '%JBN%','JBN',
if(upper(S_current.`data`.vendor_tracking_id) like '%YPM%','YEPME',
if(upper(S_current.`data`.vendor_tracking_id) like '%PTM%','PAYTM',
if(upper(S_current.`data`.vendor_tracking_id) like '%VNK%','VOONIK',
if(upper(S_current.`data`.vendor_tracking_id) like '%HOP%','HOPSCOTCH',
if(upper(S_current.`data`.vendor_tracking_id) like '%RLG%','RELIANCE_ADA',
if(upper(S_current.`data`.vendor_tracking_id) like '%PGN%','GOPIGEON','FA'))))))))),
...
if(upper(S_current.`data`.vendor_tracking_id) like '%HOP%','HOPSCOTCH',
if(upper(S_current.`data`.vendor_tracking_id) like '%RLG%','RELIANCE_ADA',
if(upper(S_current.`data`.vendor_tracking_id) like '%PGN%','GOPIGEON',
...
IF(size(S_current.`data`.associated_shipment_ids)>0 AND lower(T1.shipment_type) IN ('approved_rto','unapproved_rto','rvp') and lower(T1.status) NOT IN ('reshipped', 'pickup_leg_completed') AND lower(S_current.`data`.status) not in ('pickup_leg_completed'),'merchant_return',S_current.`data`.shipment_type) as ekl_shipment_type,
...
S_current.`data`.dispatch_service_tier as shipment_dispatch_service_tier,
CAST(S_current.`data`.dispatch_by_date  AS TIMESTAMP) as shipment_dispatch_by_datetime,
...
lzn_cons.shipment_transit_time AS shipment_transit_time,
CAST(reverse_complete_time as TIMESTAMP) as reverse_complete_datetime,
CAST(S_current.`data`.pickup_slot_start_date as TIMESTAMP) as pickup_slot_start_datetime,
CAST(S_current.`data`.pickup_slot_end_date as TIMESTAMP) as pickup_slot_end_datetime
FROM
bigfoot_snapshot.dart_wsr_scp_ekl_shipment_4_view S_current  
LEFT OUTER JOIN (SELECT `data`.pincode as src_pincode, `data`.zone as src_zone, `data`.local_territory as src_lt,`data`.city as src_city,`data`.state as src_state 
					from bigfoot_snapshot.dart_fki_scp_ekl_geo_1_view_total) geo_src ON 
					If(S_current.`data`.shipment_type = 'rvp',S_current.`data`.destination_address.pincode,S_current.`data`.source_address.pincode) = geo_src.src_pincode
LEFT OUTER JOIN (SELECT `data`.pincode as dest_pincode, `data`.zone as dest_zone, `data`.local_territory as dest_lt,`data`.city as dest_city,`data`.state as dest_state 
					from bigfoot_snapshot.dart_fki_scp_ekl_geo_1_view_total) geo_dest ON 
					If(S_current.`data`.shipment_type = 'rvp',S_current.`data`.source_address.pincode,S_current.`data`.destination_address.pincode) = geo_dest.dest_pincode
LEFT OUTER JOIN bigfoot_common.ekl_fkl_facility_mother_hub_mapping FKL_MH ON 
				(S_current.`data`.source_address.id = FKL_MH.fkl_facility_id)
LEFT OUTER JOIN bigfoot_common.ekl_fkl_facility_mother_hub_mapping Rev_FKL_MH ON 
				(S_current.`data`.destination_address.id = Rev_FKL_MH.fkl_facility_id)
LEFT OUTER JOIN (select `data`.lzn_classification AS shipment_lzn_classification,
				...
				(If(S_current.`data`.shipment_type = 'rvp',S_current.`data`.destination_address.pincode,S_current.`data`.source_address.pincode) = lzn_cons.source_pincode 
				and If(S_current.`data`.shipment_type = 'rvp',S_current.`data`.source_address.pincode,S_current.`data`.destination_address.pincode) = lzn_cons.destination_pincode)
INNER JOIN (
	...
	sum(case when `data`.shipment_type = 'forward' and `data`.status in ('Out_For_Delivery','out_for_delivery') then 1 else 0 end) as fsd_number_of_ofd_attempts,
	...
	group by entityid
	) S_group ON (S_group.shipment_id=S_current.entityid)
LEFT OUTER JOIN bigfoot_external_neo.scp_ekl__fc_profiler_volumetric_estimate_final_hive_fact Vol ON 
				(S_current.entityid = Vol.shipment_id)
LEFT OUTER JOIN
	...
	max(IF(`data`.cost_of_breach.value >= 20000, "SDD", IF(`data`.cost_of_breach.value >= 15000, 'SDD_Pilot', IF(`data`.cost_of_breach.value >= 1000, 'NDD', IF(`data`.cost_of_breach.value >= 100, 'NDD_Pilot', 'Normal'))))) AS shipment_priority_flag
	FROM bigfoot_journal.dart_wsr_scp_ekl_b2clogisticsrequest_1 LATERAL VIEW explode(`data`.ekl_reference_ids) reference_id AS refid
	where day  > date_format(date_sub(current_date,120),'yyyyMMdd')
	group by refid) b2CLogisticsRequest ON 
	(S_current.entityid = b2CLogisticsRequest.shipment_id)
LEFT OUTER JOIN bigfoot_external_neo.scp_ekl__ekl_notes_fact notes ON 
				(S_current.`data`.vendor_tracking_id=notes.vendor_tracking_id 
				AND notes.vendor_tracking_id NOT IN ('not_assigned'))
LEFT OUTER JOIN (select entityid, `data`.shipment_type as shipment_type, lower(`data`.status) as status 
				from bigfoot_snapshot.dart_wsr_scp_ekl_shipment_4_view 
				where `data`.shipment_type in ('approved_rto','unapproved_rto','rvp') 
				and lower(`data`.status) NOT IN ('reshipped', 'pickup_leg_completed')
				) T1 ON 
				(T1.entityid = S_current.`data`.associated_shipment_ids[0])
WHERE
